  ...
  return <Card>
    ...
    <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 0 }}>
      {dates.map(date => {
        ...
        return <Pressable key={localDateKey(date)} accessibilityRole="button" accessibilityLabel={`${date.toLocaleDateString("es")}${history.length ? ": entrenamiento registrado" : planned ? `: ${planned.name}` : skipped ? ": sesión quitada" : ": descanso"}`} accessibilityState={{ selected: isSelected }} onPress={() => { setSelected(date); setAdding(false); }} style={({ pressed }) => ({ width: "13.85%", minHeight: mode === "week" ? 92 : 68, borderRadius: 10, padding: 6, gap: 3, backgroundColor: isSelected ? colors.accentSoft : pressed ? colors.soft : "transparent", borderWidth: isSelected ? 1 : 0, borderColor: colors.accent, opacity: muted ? 0.38 : 1 })}>
